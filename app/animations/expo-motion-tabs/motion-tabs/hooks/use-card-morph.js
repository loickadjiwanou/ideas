...
export default function useCardMorph({ sizes, toolbarH, toolbarMinW, toolbarW, view }) {
  ...
  useEffect(() => {
    ...
    if (view === "default") {
      cardW.value =
        firstInit ? toolbarMinW : withTiming(toolbarMinW, { duration: DURATION, easing: EASING });
      cardH.value =
        firstInit ? toolbarH : withTiming(toolbarH, { duration: DURATION, easing: EASING });
      open.value =
        firstInit ? 0 : withTiming(0, { duration: DURATION - 80, easing: EASING });
      return;
    }
    ...
    if (target && target.w > 0 && target.h > 0) {
      ...
      cardW.value =
        firstInit ? targetW : withTiming(targetW, { duration: DURATION, easing: EASING });
      cardH.value =
        firstInit ? targetH : withTiming(targetH, { duration: DURATION, easing: EASING });
    }
